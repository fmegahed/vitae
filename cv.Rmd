...
urlcolor: bluelink #defined in structure.txt
linkcolor: bluelink
params:
  ...
  non_journal: !r c('Proceedings|Conference|Conferences|Workshop|Technical Paper|Virginia|Frontiers in Statistical Quality Control|arXiv|bioRxiv|PeerJ')
  ...
  conf_patterns: !r c('Proceedings|Conference|Conferences|Workshop|Technical Paper|Frontiers in Statistical Quality Control')
...
if(length(params$author_name) == 1){
  formatted_pub_names = format_publications_mod(
    pubs = pubs_tbl, author.name = params$author_name[1])
}else{
  for (i in 1:length(params$author_name)) {
    formatted_pub_names = format_publications_mod(
    pubs = pubs_tbl, author.name = params$author_name[i])
  }
}
...
- **Applications:**   `r fa('r-project')`: [rmarkdown](https://fmegahed.github.io/covid_deaths.html), [flexdashboard](http://rstudio.fsb.miamioh.edu:3838/megahefm/metric_interpretation/), [xarigan](https://fmegahed.github.io/isa401/class01/01_Introduction.html); and *gradio:* [TAVR web app](https://huggingface.co/spaces/fmegahed/tavr_project)
